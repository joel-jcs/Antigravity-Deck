...
function setupAgentWebSocket(agentWss) {
  agentWss.on("connection", (ws, req) => {
    ...
    if (!handleWsAuth(ws, req, "[WS-Agent]")) return;

    // Keep connection alive through tunnels
    setupWsHeartbeat(ws);

    let session = null;
    console.log("[WS-Agent] Connection authorized");
      ...
      try {
        switch (msg.type) {
          case "connect": {
            ...
            console.log(
              `[WS-Agent] Connected: session=${session.id.substring(0, 8)}, workspace=${session.workspace}`,
            );
            break;
          }
          ...
          case "send": {
            ...
            if (!result.text && result.busy) {
              _send(ws, {
                type: "busy_rejected",
                message: "Session is busy processing a previous message",
              });
            }
            break;
          }
          ...
          case "disconnect": {
            if (session) {
              console.log(
                `[WS-Agent] Disconnecting session ${session.id.substring(0, 8)}`,
              );
    ...
    ws.on("close", () => {
      if (session) {
        console.log(
          `[WS-Agent] Connection closed — destroying session ${session.id.substring(0, 8)}`,
        );
        sessionManager.destroySession(session.id);
        session = null;
      }
