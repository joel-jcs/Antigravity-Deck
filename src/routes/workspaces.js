...
module.exports = function setupWorkspacesRoutes(app) {
  ...
  app.get("/api/workspaces/folders", (req, res) => {
    ...
    res.json({
      root,
      folders: entries,
      suggestedWorkspaceRoot: settings.suggestedWorkspaceRoot || "",
    });
  });
  ...
  app.post("/api/workspaces/create", async (req, res) => {
    ...
    if (!folderPath && name) {
      ...
      if (!root) {
        return res.status(400).json({
          error:
            "defaultWorkspaceRoot is not configured — set it in Settings first",
        });
      }
      // Ensure root exists
      if (!fs.existsSync(root)) {
    ...
    if (!folderPath) {
      return res.status(400).json({
        error:
          "path or name is required, and defaultWorkspaceRoot must be configured",
      });
    }
    ...
    const existing = lsInstances.findIndex(
      (i) => i.workspaceFolderUri === folderUri,
    );
    if (existing >= 0) {
    ...
    if (platform === "darwin") {
      ...
      const child = spawn("antigravity", [folderPath, "--trust-workspace"], {
        timeout: 10000,
        detached: true,
        stdio: "ignore",
    ...
    while (elapsed < MAX_WAIT) {
      ...
      for (const inst of instances) {
        ...
        if (result) {
          ...
          // Dedup: background scanner might have already picked it up
          if (!lsInstances.find((i) => i.pid === inst.pid)) {
          ...
          try {
            ...
            if (result.useTls)
              addWsOpts.agent = new (require("https").Agent)({
                rejectUnauthorized: false,
              });
            await fetch(
              `${protocol}://${host}:${result.port}/exa.language_server_pb.LanguageServerService/AddTrackedWorkspace`,
              addWsOpts,
            );
          ...
          console.log(
            `[+] Workspace created: ${name} (PID: ${inst.pid}, Port: ${result.port})`,
          );
          break;
        }
      }
    ...
    if (found) {
      ...
      res.json({
        created: false,
        message: `LS not detected after ${MAX_WAIT / 1000}s. Auto-rescan will pick it up later.`,
      });
    }
  });
