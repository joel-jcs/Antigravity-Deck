...
function loadSettings() {
  ...
  _settings.suggestedWorkspaceRoot = path.join(
    os.homedir(),
  ...
  try {
    if (fs.existsSync(BRIDGE_SETTINGS_PATH)) {
      _bridgeSettings = {
        ...
        ...JSON.parse(fs.readFileSync(BRIDGE_SETTINGS_PATH, "utf-8")),
      };
    } else {
...
function saveBridgeSettings(updates) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(_bridgeSettings, null, 2),
    "utf-8",
  );
  return _bridgeSettings;
}
...
function getBridgeSettings() {
  return loadBridgeSettings();
}

// --- Agent API settings (agent-api.settings.json) ---
const AGENT_API_SETTINGS_PATH = path.join(
  __dirname,
  "..",
  "agent-api.settings.json",
);
  ...
  try {
    if (fs.existsSync(AGENT_API_SETTINGS_PATH)) {
      _agentApiSettings = {
        ...
        ...JSON.parse(fs.readFileSync(AGENT_API_SETTINGS_PATH, "utf-8")),
      };
    } else {
...
function saveAgentApiSettings(updates) {
  ...
  fs.writeFileSync(
    AGENT_API_SETTINGS_PATH,
    JSON.stringify(_agentApiSettings, null, 2),
    "utf-8",
  );
  return _agentApiSettings;
}
...
function getAgentApiSettings() {
  return loadAgentApiSettings();
}

// --- Orchestrator settings (orchestrator.settings.json) ---
const ORCHESTRATOR_SETTINGS_PATH = path.join(
  __dirname,
  "..",
...
function saveOrchestratorSettings(updates) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(_orchestratorSettings, null, 2),
    "utf-8",
  );
  return _orchestratorSettings;
}
