...
function setupOrchestratorWebSocket(wss) {
  ...
  wss.on("connection", (ws, req) => {
    ...
    if (!handleWsAuth(ws, req, "[WS-Orchestrator]")) return;
      ...
      try {
        switch (msg.type) {
          case "chat_message": {
            ...
            session
              .chat(msg.content, { intent: msg.intent, replyTo: msg.replyTo })
              .catch((e) =>
                _send(ws, {
                  type: "chat_response",
                  content: e.message,
          ...
          case "orchestrate_execute": {
            ...
            const execSession = msg.orchestrationId
              ? chatSessions.get(ws._connectionId) ||
                _getChatSessionById(chatSessions, msg.orchestrationId)
              : chatSessions.get(ws._connectionId);
            if (execSession) {
              execSession
                .chat("approve", { intent: "approve" })
                .catch((e) =>
                  _send(ws, {
                    type: "orch_error",
                    orchestrationId: execSession.id,
                    message: e.message,
                  }),
                );
              break;
          ...
          case "orchestrate_revise": {
            ...
            if (revSession) {
              if (!msg.feedback) {
                _send(ws, {
                  type: "orch_error",
                  orchestrationId: revSession.id,
                  message: 'Missing "feedback" field',
                });
                break;
              }
              revSession
                .chat(msg.feedback, { intent: "revise" })
                .catch((e) =>
                  _send(ws, {
                    type: "orch_error",
                    orchestrationId: revSession.id,
          ...
          case "orchestrate_cancel": {
            ...
            if (cancelSession) {
              cancelSession
                .chat("cancel", { intent: "cancel" })
                .catch((e) =>
                  _send(ws, {
                    type: "orch_error",
                    orchestrationId: cancelSession.id,
                    message: e.message,
                  }),
                );
              break;
          ...
          case "orchestrate_clarify": {
            ...
            if (clarifySession) {
              if (!msg.answer) {
                _send(ws, {
                  type: "orch_error",
                  orchestrationId: clarifySession.id,
                  message: 'Missing "answer" field',
                });
                break;
              }
              clarifySession
                .chat(msg.answer, { replyTo: msg.taskId })
                .catch((e) =>
                  _send(ws, {
                    type: "orch_error",
                    orchestrationId: clarifySession.id,
            ...
            if (!msg.taskId || !msg.answer) {
              _send(ws, {
                type: "orch_error",
                orchestrationId: orchClarify.id,
                message: 'Missing "taskId" or "answer" field',
              });
              break;
            }
    ...
    ws.on("close", () => {
      console.log(
        "[WS-Orchestrator] Connection closed — cleaning up subscriptions",
      );
...
function _getOrch(ws, orchestrationId) {
  if (!orchestrationId) {
    _send(ws, {
      type: "orch_error",
      message: 'Missing "orchestrationId" field',
    });
    return null;
  }
...
function _wireChatEvents(ws, session) {
  ...
  const onChatResponse = (data) =>
    _send(ws, { type: "chat_response", ...data });
