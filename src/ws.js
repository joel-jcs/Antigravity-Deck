    ...
    ws.on("message", async (raw) => {
      try {
        ...
        if (msg.type === "set_conversation") {
          ...
          console.log(
            `[WS] set_conversation → ${msg.conversationId?.substring(0, 8)}, clients: ${clientConvMap.size}`,
          );
          await ensureCached(
            msg.conversationId,
            getInstanceForCascade(msg.conversationId),
          );
          ...
          console.log(
            `[WS] subscribe_all — global viewers: ${globalViewers.size}`,
          );
