...
// Auto-config ngrok if token is present
const NGROK_TOKEN = process.env.NGROK_AUTHTOKEN;
if (NGROK_TOKEN) {
  try {
    console.log(`[*] Auto-configuring ngrok auth token...`);
    execSync(`npx ngrok config add-authtoken ${NGROK_TOKEN}`, {
      stdio: "ignore",
    });
  } catch {
    /* Ignore if it fails */
...
function killStaleProcesses(ports) {
  for (const port of ports) {
    try {
      ...
      /* No process on this port — normal */
    }
  }
}
...
function cleanup() {
  ...
  if (IS_WIN) {
    try {
      execSync(`taskkill /F /FI "IMAGENAME eq cloudflared.exe"`, {
        stdio: "ignore",
      });
    } catch {}
  } else {
    try {
      execSync(`pkill -f "cloudflared.*tunnel.*localhost"`, {
        stdio: "ignore",
      });
    } catch {}
  }
...
async function runLocal() {
  ...
  if (!needBuild) {
    ...
    try {
      ...
      if (builtPort !== String(BE_PORT)) {
        log(
          "*",
          `⚠️  Build was for port ${builtPort}, need port ${BE_PORT} — rebuilding...`,
        );
        needBuild = true;
      }
    } catch {
  ...
  if (needBuild) {
    ...
    try {
      fs.writeFileSync(portFile, String(BE_PORT));
    } catch {}
  } else {
...
async function runTunnel() {
  ...
  const authKey =
    process.env.AUTH_KEY || crypto.randomBytes(16).toString("hex");
  ...
  const tunBe = spawn(
    CLOUDFLARED,
    ["tunnel", "--url", `http://localhost:${BE_PORT}`],
    {
  ...
  beUrl = await new Promise((resolve) => {
    ...
    const handler = (data) => {
      ...
      text
        .split("\n")
        .filter((l) => l.trim())
        .forEach((l) => log("TUN-BE", l.trim()));
      // Detect Cloudflare rate limit
      if (
        buffer.includes("429") ||
        buffer.includes("Too Many Requests") ||
        buffer.includes("error code: 1015")
      ) {
  ...
  if (beUrl === "RATE_LIMITED") {
    ...
    console.error(
      "  Or use local mode: \x1b[1mnode start-tunnel.js --local\x1b[0m\n",
    );
    process.exit(1);
  }
  if (!beUrl) {
  ...
  buildFrontend({
    NEXT_PUBLIC_BACKEND_URL: beUrl,
    NEXT_PUBLIC_BACKEND_PORT: String(BE_PORT),
  });
  try {
    fs.writeFileSync(
      path.join(__dirname, "frontend", ".next", ".backend-port"),
      String(BE_PORT),
    );
  } catch {}
  ...
  const tunFe = spawn(
    CLOUDFLARED,
    ["tunnel", "--url", `http://localhost:${FE_PORT}`],
    {
      stdio: ["ignore", "pipe", "pipe"],
      shell: true,
    },
  ...
  feUrl = await new Promise((resolve) => {
    ...
    const handler = (data) => {
      ...
      if (
        ...
        console.error(
          "  Backend tunnel is working. Wait 5-10 min and try again.\n",
        );
        resolve(null);
        return;
      }
  ...
  if (feUrl) {
    console.log("\n" + "=".repeat(60));
    console.log(
      "\x1b[1m\x1b[32m  🌐 READY! Open this URL on any device:\x1b[0m",
    );
    ...
    try {
      ...
      qrcode.generate(qrUrl, { small: true }, (qr) => {
        ...
        console.log(`\n  🔗 ${qrUrl}`);
        console.log(
          `  💻 Local: \x1b[36mhttp://localhost:${FE_PORT}/?key=${authKey}\x1b[0m\n`,
        );
      });
    } catch {
...
// === Entry point ===
(LOCAL_MODE ? runLocal() : runTunnel()).catch((e) => {
  console.error(e);
  process.exit(1);
});
