...
const AnalyticsPanel = dynamic(
  () =>
    import("@/components/analytics-panel").then((m) => ({
      default: m.AnalyticsPanel,
    })),
  { ssr: false },
);
const StepDetail = dynamic(
  () =>
    import("@/components/step-detail").then((m) => ({ default: m.StepDetail })),
  { ssr: false },
);
...
export default function Home() {
  ...
  const [showAnalytics, setShowAnalytics] = useState(() =>
    getStoredValue("antigravity-show-analytics", false),
  );
  ...
  const [activeWorkspace, setActiveWorkspace] = useState<string | null>(() =>
    getStoredValue("antigravity-active-workspace", null),
  );
  ...
  const [showAccountInfo, setShowAccountInfo] = useState(() =>
    getStoredValue("antigravity-show-account-info", false),
  );
  // NEW: When true, show SettingsView in main panel
  const [showSettings, setShowSettings] = useState(() =>
    getStoredValue("antigravity-show-settings", false),
  );
  // NEW: When true, show AgentLogsView in main panel
  const [showLogs, setShowLogs] = useState(() =>
    getStoredValue("antigravity-show-logs", false),
  );
  // NEW: When true, show Agent Bridge in main panel
  const [showAgentHub, setShowAgentHub] = useState(() =>
    getStoredValue("antigravity-show-agent-hub", false),
  );
  // NEW: When true, show Connect panel in main panel
  const [showConnect, setShowConnect] = useState(() =>
    getStoredValue("antigravity-show-connect", false),
  );
  // NEW: When true, show Orchestrator standalone view
  const [showOrchestrator, setShowOrchestrator] = useState(() =>
    getStoredValue("antigravity-show-orchestrator", false),
  );
  ...
  const [activeGeminiSession, setActiveGeminiSession] = useState<string | null>(
    () => getStoredValue("antigravity-active-gemini-session", null),
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      "antigravity-active-workspace",
      JSON.stringify(activeWorkspace),
    );
  }, [activeWorkspace]);
  useEffect(() => {
    localStorage.setItem(
      "antigravity-show-settings",
      JSON.stringify(showSettings),
    );
  }, [showSettings]);
  useEffect(() => {
    localStorage.setItem(
      "antigravity-show-account-info",
      JSON.stringify(showAccountInfo),
    );
  }, [showAccountInfo]);
  useEffect(() => {
    localStorage.setItem("antigravity-show-logs", JSON.stringify(showLogs));
  }, [showLogs]);
  useEffect(() => {
    localStorage.setItem(
      "antigravity-show-agent-hub",
      JSON.stringify(showAgentHub),
    );
  }, [showAgentHub]);
  useEffect(() => {
    localStorage.setItem(
      "antigravity-show-connect",
      JSON.stringify(showConnect),
    );
  }, [showConnect]);
  useEffect(() => {
    localStorage.setItem(
      "antigravity-show-orchestrator",
      JSON.stringify(showOrchestrator),
    );
  }, [showOrchestrator]);
  useEffect(() => {
  ...
  useEffect(() => {
    localStorage.setItem(
      ...
      JSON.stringify(activeGeminiProject),
    );
  }, [activeGeminiProject]);
  useEffect(() => {
    localStorage.setItem(
      "antigravity-active-gemini-session",
      JSON.stringify(activeGeminiSession),
    );
  }, [activeGeminiSession]);
  useEffect(() => {
    localStorage.setItem(
      "antigravity-show-analytics",
  ...
  useEffect(() => {
    localStorage.setItem(
      "antigravity-current-conv-id",
      JSON.stringify(currentConvId),
    );
  }, [currentConvId]);

  useEffect(() => {
    const storedConvId = getStoredValue<string | null>(
      "antigravity-current-conv-id",
      null,
    );
    if (storedConvId) {
    ...
    getSettings()
      .then((settings) => {
        if (!settings.defaultWorkspaceRoot) {
          setSuggestedWorkspaceRoot(
            (settings as { suggestedWorkspaceRoot?: string })
  ...
  // Bookmarks
  const [bookmarkedSteps, setBookmarkedSteps] = useState<Set<number>>(
    new Set(),
  );
  ...
  const handleSelectConversation = useCallback(
    (convId: string | null, wsName: string) => {
      setActiveWorkspace(wsName);
      resetPanels();
  ...
  const handleConvListSelect = useCallback(
    (convId: string) => {
      resetPanels();
      selectConversation(convId);
    },
  ...
  // === Show Gemini ===
  const handleShowGemini = useCallback(() => {
    selectConversation(null);
    resetPanels();
    setActiveWorkspace(null);
    setShowGemini(true);
  ...
  const handleSelectGeminiProject = useCallback((projectId: string | null) => {
    setActiveGeminiProject(projectId);
    setActiveGeminiSession(null);
  }, []);

  const handleSelectGeminiSession = useCallback((sessionId: string | null) => {
    setActiveGeminiSession(sessionId);
  }, []);
  ...
  const handleCascadeCreated = useCallback(
    (cascadeId: string) => {
      setNewChatMode(false);
  ...
  const activeView = useMemo(() => {
    ...
    if (showConnect) return "connect";
    if (showSourceControl) return "source-control";
    if (showResources) return "resources";
    if (showLogs) return "logs";
    if (showSettings) return "settings";
    if (showAccountInfo) return "account";
    return "home";
  }, [
    showGemini,
    showAgentHub,
    ...
    <AuthGate>
      <div className='flex h-dvh w-full bg-background text-foreground overflow-hidden'>
        ...
        <AppSidebar
          ...
          activeGeminiSession={activeGeminiSession}
        ...
        <div className='flex flex-col flex-1 min-w-0 min-h-0 overflow-hidden pwa-main-content'>
          ...
          <header className='flex items-center justify-between px-2 sm:px-4 h-11 bg-background border-b border-border flex-shrink-0'>
              ...
              <div className='flex items-center gap-1.5'>
                ...
                {showAntigravity ? (
                  ...
                  <>
                    ...
                    <Badge
                      variant='outline'
                      className='text-[9px] h-4 px-1 font-mono inline-flex'
                    >
                      v3
                    </Badge>
                  </>
            ...
            <div className='flex items-center gap-1.5 sm:gap-3'>
              {bookmarkedSteps.size > 0 && (
                <Badge
                  variant='secondary'
                  className='text-[10px] hidden sm:inline-flex gap-1'
                >
                  <Star className='w-3 h-3' /> {bookmarkedSteps.size}
                </Badge>
              )}
              {showChat && (
              ...
              <div className='flex items-center gap-1.5'>
                ...
                <div
                  className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium ${connected ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}
                >
                  <div
                ...
                <div
                  className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium ${detected ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : connected ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-muted text-muted-foreground border border-border/30"}`}
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${detected ? "bg-emerald-400" : connected ? "bg-amber-400 animate-pulse" : "bg-muted-foreground/50"}`}
                  />
                  <span>
                    {detected
                      ? "LS Connected"
            ...
            <div className='absolute right-2 sm:right-4 top-12 z-50 w-[calc(100%-1rem)] sm:w-56 bg-popover border border-border rounded-lg shadow-xl p-3 space-y-1.5 text-xs'>
              ...
              {[
                ...
                <div key={key} className='flex items-center justify-between'>
                  ...
                  <kbd className='px-1.5 py-0.5 bg-muted/50 rounded text-[9px] font-mono text-muted-foreground'>
                    {key}
                  </kbd>
                </div>
              ))}
          ...
          {currentConvId && showTimeline && steps.length > 0 && (
            <Timeline steps={steps} onSelectStep={openStepDetail} />
          )}

          {/* Analytics — always mounted when data available, animate open/close */}
          {currentConvId && steps.length > 0 && (
            <div
              className={`grid transition-all duration-200 ease-in-out ${showAnalytics ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
            >
              ...
              {swapping ? (
                <div className='text-center space-y-4 max-w-sm'>
                  ...
                  <p className='text-sm text-muted-foreground'>
                    Closing IDE, swapping profile, and relaunching. This takes
                    ~10 seconds.
                  </p>
            ...
            <div className='flex-1 flex items-center justify-center'>
              <div className='text-center space-y-4'>
                ...
                <p className='text-sm text-muted-foreground max-w-md'>
                  Select a workspace from the sidebar to view conversations, or
                  start a new one.
                </p>
              </div>
            </div>
                ...
                <div className='flex-1 flex items-center justify-center'>
                  <div className='text-center space-y-3'>
                    ...
                    <p className='text-sm text-muted-foreground max-w-sm'>
                      Select a workspace from the sidebar first, then open
                      Source Control.
                    </p>
                  </div>
                </div>
          ...
          {showChat && (
            <>
              ...
              <StepDetail
                ...
                isBookmarked={
                  selectedStep !== null
                    ? bookmarkedSteps.has(selectedStep)
                    : false
                }
          ...
          {pwaInstall.showBanner && (
            <div className='flex items-center gap-2 px-3 py-2 bg-blue-500/10 border-t border-blue-500/20 flex-shrink-0'>
              ...
              <span className='text-xs text-blue-300 flex-1'>
                {pwaInstall.isIOS ? (
                  <>
                    Tap <Share2 className='w-3 h-3 inline' /> then &quot;Add to
                    Home Screen&quot;
                  </>
                ) : (
          ...
          <footer className='flex items-center justify-between px-2 sm:px-4 h-8 bg-background border-t border-border flex-shrink-0 text-[10px] text-muted-foreground/60'>
            <div className='flex items-center gap-2 sm:gap-3'>
              <span>
                <FolderSync className='w-3 h-3 inline-block mr-1' />
                Antigravity Deck v3
              </span>
            ...
            <div className='flex items-center gap-2 sm:gap-3'>
              ...
              >
                <kbd className='px-1 py-0.5 bg-muted/50 rounded text-[9px] font-mono'>
                  ?
                </kbd>
                <span>Shortcuts</span>
              </button>
