...
// Four WebSocket servers: UI (/ws), Agent API (/ws/agent), Orchestrator (/ws/orchestrator), Gemini (/ws/gemini)
...
morgan.token("redacted-referrer", (req) =>
  redactSensitive(req.get("referrer")),
);
...
app.use((req, res, next) => {
  ...
  res.header(
    ...
    "Content-Type, X-Auth-Key, Authorization, Accept, Origin, X-Requested-With",
  );
  res.header(
...
const { AUTH_KEY, isNoAuth } = require("./src/config");
if (AUTH_KEY) {
  ...
  app.use("/api", (req, res, next) => {
    ...
    if (!key || key.length !== AUTH_KEY.length) {
      return res
        .status(401)
        .json({ error: "Unauthorized — invalid or missing auth key" });
    }
...
// Gemini WebSocket — gemini protocol at /ws/gemini
const { setupGeminiWebSocket } = require("./src/ws-gemini");
setupGeminiWebSocket(geminiWss);
...
server.listen(PORT, async () => {
  ...
  if (
    bridgeCfg.autoStart &&
    bridgeCfg.discordBotToken &&
    bridgeCfg.discordChannelId
  ) {
